...
// SSE3 SIMD-оптимизированная версия batch intersection
// Обрабатывает до 4 треугольника параллельно
    ...
    float* outV) noexcept
{
  ...
  const __m128 v1x = _mm_load_ps(batch.v1x);
  const __m128 v1y = _mm_load_ps(batch.v1y);
  const __m128 v1z = _mm_load_ps(batch.v1z);
  const __m128 v2x = _mm_load_ps(batch.v2x);
  const __m128 v2y = _mm_load_ps(batch.v2y);
  const __m128 v2z = _mm_load_ps(batch.v2z);
