...
// AVX2 SIMD-оптимизированная быстрая проверка пересечения
// Возвращает true при первом попадании (без вычисления barycentric)
    ...
    bool backfaceCulling) noexcept
{
  ...
  // Check if any of the first batch.triangleCount triangles hit
  const int validMask = maskInt & ((1 << batch.triangleCount) - 1);
  return validMask != 0;
}
